...
function generateUser() {
  ...
  const subjects = ['English', 'Chemistry', 'Computer Science', 'Commerce', 'Economics', 'Social Studies'];
...
function generateCity(state) {
  ...
  switch(state) {
    case 'NCR': return generateSomeOption(cityForNcr, cityForNcr.length);
    case 'Uttar Pradesh': return generateSomeOption(cityForUttar, cityForUttar.length);
    case 'Haryana': return generateSomeOption(cityForHaryana, cityForHaryana.length);
    case 'Rajasthan': return generateSomeOption(cityForRajasthan, cityForRajasthan.length);
  }
}
...
function generateBirthDate() {
  ...
  const randomYear = Math.floor(Math.random() * (maxPossibleYear - minPossibleYear + 1)) + minPossibleYear;
